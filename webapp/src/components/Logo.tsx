...
export function KMarkIcon({ size = 24 }: { size?: number }) {
  return (
...
export function LogoMark({ size = 46, light = false, className = '', style }: LogoMarkProps) {
  return (
    <div className={`logo ${light ? 'light' : ''} ${className}`} style={{ width: size, height: size, ...style }}>
...
export function Logo({
  ...
  return (
    <div className={`brand ${className}`} onClick={onClick} style={style}>
