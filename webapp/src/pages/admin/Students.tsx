...
export default function Students() {
  ...
  return (
    <Card
      ...
      action={<Input placeholder="ค้นหาชื่อ…" value={q} onChange={(e) => setQ(e.target.value)} style={{ width: 230 }} />}
    >
      <Table
        heads={['นักเรียน', 'อายุ · การศึกษา', 'แพ็กเกจ', 'ชั่วโมงคงเหลือ', 'เรียนแล้ว', 'สถานะ']}
        rows={filtered.map((s) => [
          <b key="n">{s.name}</b>,
          s.info,
          ...
          `${s.done} คลาส`,
          s.state === 'active' ? (
            <Badge key="st" tone="green">ใช้งาน</Badge>
          ) : s.state === 'new' ? (
            <Badge key="st" tone="blue">ใหม่</Badge>
          ) : (
            <Badge key="st" tone="gray">แพ็กเกจหมดอายุ</Badge>
          ),
        ])}
      />
    </Card>
