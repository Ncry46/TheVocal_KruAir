...
export default function Requests() {
  const { toast } = useApp()
  const [requests, setRequests] = useState<MoveRequest[] | null>(null)

  useEffect(() => {
    api.getMoveRequests().then(setRequests)
  }, [])

  const decide = async (r: MoveRequest, approve: boolean) => {
    await api.decideMove(r.id, approve)
    setRequests((prev) => prev?.map((x) => (x.id === r.id ? { ...x, status: approve ? 'อนุมัติแล้ว' : 'ปฏิเสธ' } : x)) ?? null)
    toast(
      approve ? `อนุมัติเลื่อนนัดของ ${r.student} แล้ว · ส่งแจ้งเตือนให้นักเรียน (เว็บ + LINE)` : `ปฏิเสธคำขอของ ${r.student} · แจ้งนักเรียนแล้ว`,
      'ok',
    )
  }
  ...
  return (
    <>
      ...
      <Card title="คำขอเลื่อนนัด">
        <Table
          heads={['นักเรียน', 'นัดเดิม', 'ขอนัดใหม่', 'ขอเมื่อ', 'สถานะ', 'การดำเนินการ']}
          rows={requests.map((r) => [
            <b key="n">{r.student}</b>,
            r.from,
            ...
            r.at,
            r.status === 'รออนุมัติ' ? (
              <Badge key="st" tone="pink">รออนุมัติ</Badge>
            ) : r.status === 'อนุมัติแล้ว' ? (
              <Badge key="st" tone="green">อนุมัติแล้ว</Badge>
            ) : (
              <Badge key="st" tone="red">ปฏิเสธ</Badge>
            ),
            r.status === 'รออนุมัติ' ? (
              ...
              <Badge key="a" tone="blue">แจ้งนักเรียนแล้ว</Badge>
            ),
          ])}
        />
      </Card>
