  ...
  return (
    <>
      <Card
        ...
        action={
          <Button pink size="sm" onClick={() => setOpen(true)}>
            ＋ สร้างวอเชอร์
          </Button>
        }
      >
        <Table
          heads={['โค้ด', 'ประเภท', 'ใช้ได้ถึง', 'ใช้แล้ว / ทั้งหมด', 'สถานะ', '']}
          rows={rows.map((v) => [
            ...
            v.state === 'active' ? (
              <Button key="a" danger size="sm" onClick={() => toast(`ปิดวอเชอร์ ${v.code} แล้ว`, 'ok')}>ปิด</Button>
            ) : (
              <Button key="a" ghost size="sm" onClick={() => toast('เปิดใช้งาน (จำลอง)', 'ok')}>เปิด</Button>
            ),
          ])}
        />
      </Card>
