...
import type { DaySlot, LessonHistory, MyLesson, PackageStatus } from '../../lib/types'
...
export default function Home() {
  ...
  const [hist, setHist] = useState<LessonHistory[] | null>(null)
  ...
  useEffect(() => {
    ...
    setSlots(null)
    setMoveTime('')
    if (moveDay) api.getSlots(moveDay).then(setSlots)
  }, [moveDay, moveOpen])

  if (!status || !hist || !lessons) return <Skeleton />
  ...
  const submitMove = async () => {
    if (!next || !moveDay || !moveTime || moveBusy) return
    setMoveBusy(true)
    try {
  ...
  return (
    <>
      ...
      <div className="card hero-card">
        ...
        <div className="hero-actions">
          {next && next.status === 'pending' && (
            <Button green onClick={confirmNext} disabled={confirming}>
              {confirming ? 'กำลังยืนยัน…' : 'ยืนยันการมาเรียน'}
            </Button>
          )}
          {next && next.status !== 'moved' && (
            <Button ghost onClick={openMove}>ขอเลื่อนนัด</Button>
          )}
      ...
      <Modal open={moveOpen} onClose={() => setMoveOpen(false)} title="ขอเลื่อนนัด">
        ...
        <Field label="เลือกวันใหม่">
          <div className="chip-row">
            {days.map((d) => (
              <button key={d} className={`dchip ${moveDay === d ? 'on' : ''}`} onClick={() => setMoveDay(d)}>
                {d}
              </button>
            ))}
          </div>
              ...
              <div className="slots-grid">
                {slots.map((s) => (
                  <button
                    ...
                    className={`slot-btn ${moveTime === s.time ? 'on' : ''}`}
        ...
        <Button
          ...
          disabled={!moveDay || !moveTime || moveBusy}
          onClick={submitMove}
        >
      ...
      <div className="grid cols-2">
        <Card
          title="ประวัติล่าสุด"
          action={<Link to="/app/history" className="link">ดูทั้งหมด →</Link>}
        >
          <Table
            heads={['วันที่', 'คลาส', 'บันทึกครู']}
        ...
        <Card
          title="แพ็กเกจของฉัน"
          action={<Link to="/app/packages" className="link">ซื้อเพิ่ม →</Link>}
        >
